...
if (!fs.existsSync(gateway)) {
  ...
  let output = execSync(
    'git init && git remote add -f origin https://github.com/scayle/gateway',
    { encoding: 'utf-8', cwd: gateway }
  )
  console.log('fetching gateway:\n' + output)

  output = execSync('git config core.sparseCheckout true', {
    encoding: 'utf-8',
    cwd: gateway
  })
  console.log(output)

  output = execSync('echo "graph/definition/" >> .git/info/sparse-checkout', {
    encoding: 'utf-8',
    cwd: gateway
  })
  console.log(output)
...
let output = execSync(`git checkout ${branch} && git pull`, {
  encoding: 'utf-8',
  cwd: gateway
})
console.log(output)
